...
const LabManual = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (submission && submission._id) {
                // UPDATE EXISTING: Use observations from the state
                const res = await axios.patch(`http://localhost:5001/api/submissions/${submission._id}`, {
                    observations: observations, // This ensures the 'ok done' text is sent
                    status: 'Pending', 
                    updatedAt: new Date()
                });
                updatedSubmission = res.data;
                alert('🔄 Record updated and sent to faculty!');
            } else {
                // CREATE NEW
                const res = await axios.post('http://localhost:5001/api/submissions', {
                    labId: id,
            ...
            // CRITICAL: Update local state immediately so PDF and UI sync
            setSubmission(updatedSubmission);
            setObservations(updatedSubmission.observations);

            // Optional: window.location.reload() ensures everything is clean for the sidebar
    ...
    const downloadPDF = () => {
        ...
        // FIX: Always use 'observations' state to ensure PDF uses what the student just typed
    ...
    const isLocked = submission && submission.status === 'Approved';
            ...
            <div style={styles.layout}>
                ...
                <div style={styles.mainContent}>
                    {selectedExp ? (
                        <div style={{ maxWidth: '900px', margin: 'auto' }}>
                            <div style={styles.mainHeader}>
                                ...
                                {submission?.status === 'Approved' && (
                                    <div style={styles.gradeBadge}>Grade: {submission.grade}/100</div>
                                )}
                            </div>
                                ...
                                {isLocked ? (
                                    <div style={styles.box}>
                                        ...
                                        <div style={styles.lockedFooter}>
                                            ...
                                            {submission.status === 'Approved' && (
                                                <button onClick={downloadPDF} style={styles.downloadBtn}>📄 Download Official PDF</button>
                                            )}
                                        </div>
                                    </div>
                                ) : (
                                    <form onSubmit={handleSubmit} style={styles.box}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '16px', gap: '10px' }}>
                                            <button type="submit" style={submission?.status === 'Pending' ? { ...styles.submitBtn, background: '#64748b' } : styles.submitBtn}>
                                                {submission?.status === 'Pending' ? '🔄 Update Submission' : submission?.status === 'Redo' ? '🔄 Resubmit Correction' : '📤 Submit for Review'}
                                            </button>
                                        </div>
                                    </form>
                                )}
                        ...
                        <div style={styles.emptyState}><h2>Select an experiment from the sidebar to begin.</h2></div>
                    )}
                </div>
            </div>
        </div>
...
const styles = {
    ...
    sidebar: { width: '280px', background: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(6px)', padding: '24px', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', overflowY: 'auto' },
    backBtn: { background: '#f1f5f9', color: '#475569', border: '1px solid #e2e8f0', padding: '8px 15px', borderRadius: '8px', cursor: 'pointer', marginBottom: '24px', width: '100%', fontWeight: '600', transition: '0.2s', textAlign: 'center' },
    ...
    experimentBtn: (isActive) => ({ display: 'block', width: '100%', textAlign: 'left', padding: '12px 14px', borderRadius: '10px', border: 'none', fontSize: '14px', cursor: 'pointer', transition: '0.2s', background: isActive ? 'linear-gradient(90deg, #dbeafe, #eff6ff)' : 'transparent', color: isActive ? '#1e3a8a' : '#374151', fontWeight: isActive ? '600' : 'normal', }),
    ...
    statusBanner: (status) => ({ padding: '16px', borderRadius: '12px', marginBottom: '24px', border: '1px solid', background: status === 'Approved' ? '#dcfce7' : status === 'Redo' ? '#fee2e2' : '#fef9c3', borderColor: status === 'Approved' ? '#bbf7d0' : status === 'Redo' ? '#fecaca' : '#fef08a', color: status === 'Approved' ? '#166534' : status === 'Redo' ? '#991b1b' : '#854d0e', }),
    ...
    textArea: { width: '100%', minHeight: '220px', padding: '16px', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '15px', outline: 'none', transition: 'all 0.2s', fontFamily: 'monospace', background: '#f8fafc', color: '#1f2937', resize: 'vertical' },
    submitBtn: { background: '#3b82f6', color: 'white', border: 'none', padding: '12px 28px', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold', fontSize: '15px', boxShadow: '0 4px 12px rgba(59, 130, 246, 0.3)', transition: '0.2s' },
    ...
    downloadBtn: { background: '#1e3a8a', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600', transition: '0.2s', boxShadow: '0 4px 6px rgba(30, 58, 138, 0.2)' }
};
